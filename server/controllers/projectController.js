...
const addTeamMemberToProject = async (req, res) => {
  ...
  try {
    const existEmail = await authSchema.findOne({
      email
    });
    if (!existEmail) return res.status(400).send({
    ...
    if (existMember) return res.status(400).send({
    ...
    res.status(200).send({
      message: "Team Mebmer added successfylly"
    })
  } catch (error) {
    console.log(error);
    res.status(500).send({
...
const addTaskToProject = async (req, res) => {
  ...
  try {
    ...
    const projectData = await projectSchema.findOneAndUpdate(
      { _id: projectId }, { tasks: { title, description, priority, assignedTo } },
